...
#                     result = data['Adj Close'].to_frame(clean_tickers[0])
#                 else:
#                     result = data.to_frame(clean_tickers[0])
#                 print(f"Single ticker data shape: {result.shape}")
#                 return result
#             else:
#                 if isinstance(data.columns, pd.MultiIndex):
#                     result = data['Adj Close']
#                     print(f"Multi ticker data shape: {result.shape}")
#                     return result
#                 else:
...
#                 print(f"Dimension mismatch: weights={len(weights)}, columns={len(returns.columns)}")
#                 if len(returns.columns) >= len(weights):
#                     returns = returns.iloc[:, :len(weights)]
#                     print(f"Trimmed returns to match weights: {returns.shape}")
#                 else:
#                     print("Not enough return columns for given weights")
#                     return None, None
...
#                 print("Portfolio returns are empty after cleaning")
#                 return None, None
...
#             weights: List of weights (0-1 range, should sum to 1) OR investment amounts
...
#                 return self._create_minimal_result("Missing tickers or weights")

#             if len(tickers) != len(weights):
#                 return self._create_minimal_result("Length mismatch between tickers and weights")

#             # Convert weights to numpy array
#             weights_array = np.array(weights, dtype=float)
            
#             if weights_array.sum() == 0:
#                 return self._create_minimal_result("Total weight is zero")

#             # If weights don't sum to 1, assume they are investment amounts and normalize
#             if not np.isclose(weights_array.sum(), 1.0, rtol=1e-3):
#                 print(f"Normalizing weights from {weights_array.sum()} to 1.0")
#                 weights_array = weights_array / weights_array.sum()

#             # Get stock data
#             stock_data = self.get_stock_data(tickers, start_date, end_date)
#             if stock_data is None or stock_data.empty:
#                 return self._create_minimal_result("Stock data not available")

#             # Calculate returns
#             returns = self.calculate_returns(stock_data)
#             if returns is None or returns.empty:
#                 return self._create_minimal_result("Returns not available")

#             # Calculate portfolio metrics
#             metrics, portfolio_returns = self.calculate_portfolio_metrics(returns, weights_array)
#             if metrics is None:
#                 return self._create_minimal_result("Metrics calculation failed")
...
#                             benchmark_returns = benchmark_returns.iloc[:, 0]  # Get first column
...
#             # Calculate individual asset metrics for additional analysis
#             individual_metrics = {}
...
#             import traceback
#             traceback.print_exc()
#             return self._create_minimal_result(f"Analysis failed: {str(e)}")

#     def _create_minimal_result(self, error_message):
#         """Create a minimal result structure for error cases"""
#         print(f"Creating minimal result due to: {error_message}")
#         return {
#             'metrics': {
#                 'expected_return': 0.0,
#                 'volatility': 0.0,
